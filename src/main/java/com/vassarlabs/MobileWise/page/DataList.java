...
public class DataList {
    ...
    private By lhsAddField = By.xpath("//h6[text()='LHS']/parent::div//button[@color='primary']");
    private By lhsFielddeleteButton = By.xpath("//h6[text()='LHS']/../div/div/div/button");
    private By rhsAddField = By.xpath("//h6[text()='RHS']/parent::div//button[@color='primary']");
        ...
        if (!(driver.findElements(By.xpath("//h6[text()='LHS']/parent::div//button[@disabled='true']")).size() > 0)) {
            for (int i = 0; i <= totalFields.length - 1; i++) {
                for (int j = 0; j <= lhsFields.length - 1; j++) {
                    System.out.println("Lhs Ittration " + j);
                    driver.findElement(lhsAddField).click();
                    driver.findElements(lhsSelectField).get(j).click();
                    Thread.sleep(3000);
...
//                        driver.findElements(lhsFielddeleteButton).get(j).click();
                ...
                for (int j = 0; j <= rhsFields.length - 1; j++) {
                    ...
                    driver.findElements(rhsSelectField).get(j).click();
...
//                        driver.findElements(rhsFielddeleteButton).get(j).click();
    ...
    public void dataListDisplayViewToggles(String values) throws Throwable {
        Thread.sleep(3000);
        String[] value = values.split(",");
        String[] toggles = value[value.length-1].split("-");
        for (int i = 0; i <= toggles.length - 1; i++) {
            String[]PageValues = toggles[i].split("_");
            if (driver.findElements(By.xpath("//p[text()='" + PageValues[0] + "']/parent::div//button")).size() > 0) {
                boolean expected = Boolean.parseBoolean(PageValues[1]);
                System.out.println(expected);
                if (expected) {
                    driver.findElement(By.xpath("//p[text()='" + PageValues[0] + "']/parent::div//button")).click();
