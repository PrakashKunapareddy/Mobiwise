...
public class UserProfile {
    ...
    private By usernameInUserProfile = By.xpath("//div[@class='buttons']/div/div");
    ...
    private By changePasswordFieldError = By.xpath("//div[contains(@class,'mat-mdc-text-field-wrapper')]/div/div/input[@placeholder='Enter New Password']/../../../following-sibling::div/div/mat-hint/text()");
    private By confirmPasswordFieldError = By.xpath("//div[contains(@class,'mat-mdc-text-field-wrapper')]/div/div/input[@placeholder='Confirm Password']/../../../following-sibling::div/div/mat-hint/text()");
   ...
   //Check username which is displayed on the Home page to verify it with logged in username
    public void checkUserName(String username) throws Throwable {
        Thread.sleep(3000);
        String userVerify = driver.findElement(usernameInUserProfile).getText();
        Assert.assertEquals(username, userVerify, "Expected Error Message " + username + " But Found : " + userVerify);
    ...
    public void checkUserProfile(String list_option1, String list_option2) throws Throwable {
        if (verifyUserFlag) {
            ...
            Thread.sleep(3000);
            String changePasswordTextInUserProfile = driver.findElement(changePasswordUserProfile).getText();
            ...
            Thread.sleep(2000);
            String logOutTextInUserProfile = driver.findElement(changePasswordUserProfile).getText();
...
//            Matcher matcher = pattern.matcher(new_password);
            ...
            if(driver.findElements(confirmPasswordFieldError).size()>0){
                String message = driver.findElement(confirmPasswordFieldError).getText();
    ...
    public void verifytoasterChangePassword(String toaster_message)throws Throwable{
        if(submitFlag){
            ...
            if(driver.findElements(successfulToasterChangeOfPassword).size()>0){
                String message =driver.findElement(successfulToasterChangeOfPassword).getText();
...
public void validateLogout(String verify_logout) throws Throwable {
        if(loginFlag){
            Thread.sleep(3000);
            driver.findElement(userProfileExpandIcon).click();
           if(driver.findElements(logOutUserProfile).size()>0){
               Thread.sleep(1000);
